...
function getProducts() {
  if(fs.existsSync(PROD_FILE)){try{return JSON.parse(fs.readFileSync(PROD_FILE,"utf8"));}catch(e){}}
  return [];
}
...
app.get("/get-orders", (req, res) => {
  ...
  const ORDER_FILE = path.join(__dirname, "vetraj_orders.json");
  try{
    const orders = fs.existsSync(ORDER_FILE) ? JSON.parse(fs.readFileSync(ORDER_FILE,"utf8")) : [];
    res.json({ orders });
